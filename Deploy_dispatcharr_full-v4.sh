...
#
# Options:
#   --no-swap        Skip creating/using the /swapfile (step 1). Use this if
#                     the box already has enough RAM, already manages its own
#                     swap, or is a container/VM where swapfiles aren't wanted
#                     (e.g. some LXC setups). The frontend build step still
#                     gets the large Node heap either way — it'll just have
#                     to fit in real RAM without swap to fall back on, so
#                     only skip this on a box with enough memory to spare.
#   SKIP_SWAP=1       Same effect as --no-swap, set as an env var instead —
#                     handy for non-interactive/remote runs, e.g.:
#                       SKIP_SWAP=1 bash deploy_dispatcharr_full.sh
#   -y, --yes        Skip the settings summary/confirmation prompt (and the
#                     swap Y/n prompt if --no-swap/SKIP_SWAP wasn't given
#                     either) and just proceed with the defaults shown.
#
...
SWAP_FLAG_SET=0        # did the user explicitly decide swap on/off via flag or env?
[[ "$SKIP_SWAP" == "1" ]] && SWAP_FLAG_SET=1
ASSUME_YES=0
for arg in "$@"; do
  case "$arg" in
    --no-swap) SKIP_SWAP=1; SWAP_FLAG_SET=1 ;;
    -y|--yes) ASSUME_YES=1 ;;
    -h|--help)
      ...
      ;;
    *)
      echo "[ERROR] Unknown option: $arg (use --no-swap, -y/--yes, or -h)" >&2
      exit 1
      ;;
  esac
done
...
# If the user didn't already decide swap on/off via --no-swap or SKIP_SWAP=1,
# and we can actually ask (a real terminal, not a pipe/cron/CI run), ask now.
if [[ "$SWAP_FLAG_SET" == "0" && "$ASSUME_YES" == "0" && "$INTERACTIVE" == "1" ]]; then
  read -r -p "Create a ${SWAP_SIZE_GB} GB swapfile at /swapfile? [Y/n] " ans
  case "${ans:-Y}" in
    [nN]*) SKIP_SWAP=1 ;;
    *) SKIP_SWAP=0 ;;
  esac
...
if [[ "$SKIP_SWAP" == "1" ]]; then
  SWAP_PLAN="disabled — no /swapfile will be created or touched"
else
  SWAP_PLAN="${SWAP_SIZE_GB} GB /swapfile will be created (or reused if already active) and added to /etc/fstab"
fi

cat <<SUMMARY

==================================================================
 Dispatcharr install — settings
==================================================================
 Swap:              ${SWAP_PLAN}
 Node build heap:   ${NODE_HEAP_MB} MB  (passed to the frontend's 'npm run build')
 Repo clone dir:    ${CLONE_DIR}
 App install dir:   ${APP_DIR}
 Postgres DB/user:  ${POSTGRES_DB} / ${POSTGRES_USER}
 Postgres password: ${POSTGRES_PASSWORD}$( [[ "$POSTGRES_PASSWORD" == "secret" ]] && echo "  (default — change POSTGRES_PASSWORD in this script before using this for anything real)" )
 Web UI port:       ${HTTP_PORT}
 WebSocket port:    ${WS_PORT}
 Locale ensured:    ${LOCALE}

 Beyond what Dispatcharr's own official installer does, this script also:
   - Forces /usr/sbin, /sbin, etc. onto PATH before doing anything, since
     some shells (su without '-', 'pct enter', minimal containers) hand us
     a PATH missing those directories even when the binaries are present.
   - Installs and verifies 'util-linux-extra' (mkswap/swapon — split out of
     the base util-linux package on Debian trixie) and 'locales', then
     double-checks with 'command -v' that they actually resolve rather than
     trusting apt's "already installed" message.
   - If a required command still isn't found, looks it up with 'dpkg -L' and
     symlinks it onto PATH itself, instead of just failing.
   - Generates and activates the ${LOCALE} locale up front, so the official
     installer's own locale-gen call has something to work with.
   - Patches the official installer's frontend build line to use a
     ${NODE_HEAP_MB} MB Node heap, and rebuilds the frontend manually
     afterward if the build still didn't produce output.
   - Treats a failed mkswap/swapon as fatal (stops the script) instead of
     printing "Swap enabled" regardless of whether it actually was.
==================================================================
SUMMARY
...
if [[ "$ASSUME_YES" == "0" ]]; then
  if [[ "$INTERACTIVE" == "1" ]]; then
    ...
    echo "(Not an interactive terminal — proceeding automatically. Pass --no-swap, -y, or edit the tunables at the top of this script to change the above.)"
  fi
fi
...
if [[ "$SKIP_SWAP" == "1" ]]; then
  apt install -y git curl locales
else
  apt install -y git curl util-linux-extra locales
fi

log "Verifying required tooling is actually present..."
if [[ "$SKIP_SWAP" == "1" ]]; then
  echo "  --no-swap set: skipping mkswap/swapon checks."
else
...
########################################
if [[ "$SKIP_SWAP" == "1" ]]; then
  log "[1/8] --no-swap set: skipping swap setup."
  if swapon --show | grep -q '/swapfile'; then
    echo "Note: a /swapfile from a previous run is still active. This script"
    echo "won't touch it — remove it yourself with 'swapoff /swapfile &&"
    echo "rm /swapfile' and drop the /swapfile line from /etc/fstab if you"
    echo "want it gone too."
  fi
  echo "Make sure this box has enough real RAM for the frontend build"
  echo "(~${NODE_HEAP_MB} MB heap) without swap to fall back on."
else
